...
<div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
    ...
    <div class="md:col-span-2 bg-white p-6 rounded shadow-md space-y-4">
        {{-- Post title --}}
        <h1 class="text-3xl font-bold text-[#4B0082]">{{ $post->title }}</h1>

        {{-- Category --}}
        <p class="text-gray-500">Category: <span class="text-[#4B0082] font-medium">{{ $post->category }}</span></p>

        {{-- Image --}}
        @if($post->image)
            <img src="{{ asset('storage/'.$post->image) }}" alt="Post Image" class="w-full h-64 object-cover rounded border border-[#4B0082] shadow-sm">
        @endif
        ...
        <div class="flex space-x-2 mt-4">
            ...
            <a href="{{ route('posts.edit', $post->id) }}" class="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600">
                Edit Post
            </a>
